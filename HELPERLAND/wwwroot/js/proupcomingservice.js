const dashboard = document.querySelector("#upcomingservice");
...
async function completerequest(serviceId) {

    try {
        if (serviceId) {
            body.classList.add("loading");
            const res = await fetch(`/Provider/CompleteRequest?serviceId=${serviceId}`, { method: "GET" });
            const data = await res.json();

            if (data.responce == 1) {
                errorspan.innerHTML = `<div class="alert alert-success alert-dismissible fade show" role="alert">
                <strong>Success! </strong>Your input has been successfully saved.
                ...
                <strong>Failed! </strong>Sorry for inconvinience your input has't been stored.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>`;
            }
        }
...
async function cancelrequest(serviceId) {

    try {
        if (serviceId) {
            body.classList.add("loading");
            const res = await fetch(`/Provider/CancelRequest?serviceId=${serviceId}`, { method: "GET" });
            const data = await res.json();

            if (data.responce == 1) {
                errorspan.innerHTML = `<div class="alert alert-danger alert-dismissible fade show" role="alert">
                <strong>Cancelled! </strong>Now this service does not belong to you.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>`;
                setTimeout(() => {
                    window.location.reload();
